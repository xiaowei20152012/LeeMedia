...
/**
 */
public class FileImageFragment extends FilesPagerRecyclerViewCustomGridSizeFragment<VideoAdapter, GridLayoutManager> implements LoaderManager.LoaderCallbacks<ArrayList<FileInfo>> {

    public static final String TAG = FileImageFragment.class.getSimpleName();

    private static final int LOADER_ID = LoaderIds.FILE_IMAGE_FRAGMENT;
    ...
    private static class AsyncFilesLoader extends WrappedAsyncTaskLoader<ArrayList<FileInfo>> {
        ...
        public ArrayList<FileInfo> loadInBackground() {
            if (null == LocalFileDataSource.getInstance().getImageInfos()) {
                return LocalFileDataSource.getInstance().getNormalInfos(getContext());
            }
            return LocalFileDataSource.getInstance().getImageInfos();
        }
    }
}
