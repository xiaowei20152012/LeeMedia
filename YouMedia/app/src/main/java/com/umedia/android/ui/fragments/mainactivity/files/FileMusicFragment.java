...
/**
 */
public class FileMusicFragment extends FilesPagerRecyclerViewCustomGridSizeFragment<VideoAdapter, GridLayoutManager> implements LoaderManager.LoaderCallbacks<ArrayList<FileInfo>> {

    public static final String TAG = FileMusicFragment.class.getSimpleName();

    private static final int LOADER_ID = LoaderIds.FILE_MUSIC_FRAGMENT;
    ...
    private static class AsyncFilesLoader extends WrappedAsyncTaskLoader<ArrayList<FileInfo>> {
        ...
        public ArrayList<FileInfo> loadInBackground() {
            if (null == LocalFileDataSource.getInstance().getAudioInfos()) {
                return LocalFileDataSource.getInstance().getNormalInfos(getContext());
            }
            return LocalFileDataSource.getInstance().getAudioInfos();
        }
    }
}
